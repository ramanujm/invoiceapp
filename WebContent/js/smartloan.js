...
//	alert('pairs is null = ' + ( pairs == null ));
	...
	if ( pairs != null ) {
		...
		for ( i = 0; i < pairs.length; i++ ) {
			if ( pairs[i] ) {
				...
				var value = unescape(getItemValue(pairs[i]));
...
//						alert("found!");
						...
						{ 
							element.value = unescape(valueArr[i]);
...
//	window.open().document.write(data);
	...
	if ( offset != -1 ) {
		...
		return unescape(data.substring(offset+len+1, offset2));
...
/*	if ( getCookie(cookieName) != null ) 
	{
		delEatCookie(cookieName);
	}*/

	var expire = new Date();
	expire.setTime (expire.getTime() + cookieStoreDays * 24 * 60 * 60 * 1000);
	document.cookie = cookieName + "=" + escape(cookieVal) + "; expires="+expire.toGMTString();   // <-- Permanent Cookie
//	document.cookie = cookieName + "=" + cookieVal;	// <-- Session Cookie
}
...
//		alert("itemName = " + itemName + "; oldValue = " + oldValue + "; newValue = " + itemValue);
//		alert("cookieName = " + cookieName + "; itemName = " + itemName + "; itemIndex = " + itemIndex);
...
//	alert("cookee = " + cookee);
	...
	//alert("cookie blank = " + (cookee == ''));
	//window.open("","", "height=200, width=450, scrollbars=yes").document.write("<html><body><p>"+cookee+"</p></body></html>");
}
...
function clearImg(y)
{
	if(browTest()==true){
		OrImg= new Image();
		if(y==1){
			OrImg.src="https://www.secureapp.com/b2bbank/graphics/qualback_off.gif";
			document.images.Back_button.src = OrImg.src;
		}
		else if(y==2){
			OrImg.src="https://www.secureapp.com/b2bbank/graphics/next_off.gif";
...
function makeImg(y)
{
	if(browTest()==true){
		Img = new Image();
		if(y==1){
			Img.src = "https://www.secureapp.com/b2bbank/graphics/qualback_on.gif"; 
			document.images.Back_button.src = Img.src;
		}
		else if(y==2){
			Img.src = "https://www.secureapp.com/b2bbank/graphics/next_on.gif"; 
...
function loadImg()
{
	if(browTest()==true) {
		...
		Img.src = "https://www.secureapp.com/b2bbank/graphics/qualback_on.gif"
		Img2 = new Image();
		Img2.src = "https://www.secureapp.com/b2bbank/graphics/next_on.gif";
	}
	else {	}
}
...
{
//	alert("currencyStr = " + currencyStr);
//	String currencyStr = currStr+"";
...
function checkMandatoryFields()
{
  ...
  var mortgage = new Array("fLoanPurp", "fHomeValue", "fMortgageAmountRequested", "fApproxDate", "fMrtType");
	var appMand = new Array( "FirstName","LastName", "DOB", "Address1", "Address2", "CurrentCity", "CurrentProvince", 
								"CurrentCode", "Accommodation", "HomePhone", "CurrentAddrTime", "Occupation", "PresEmpName", "LengthService", 
								"AnnualIncome", "TypeOfIncome");
  */
  
  var mortgage = new Array("fOfficeName");
	var appMand = new Array( "FirstName","LastName", "DOB", "Address1", "Address2", "CurrentCity", "CurrentProvince", 
								"CurrentCode", "Accommodation", "HomePhone");							
	...
	try {
  	for(var i=0; i<liabilities.length; i++)
  	{
  		...
  		if ( amount != "" && amount != 0 && payment == 0)
  		{
  			alert("Liabilities owing must have monthly mortgage payment entered.");
  			return false;
  		}
  	}
  }
  catch(e) {
    // Missing element -- must be using short form -- ignore
  }
	
	return true;
}

function checkMonths(field, label) {
  var value = field.value * 1.0
  if ( value < 0 || value > 11 ) {
    alert("Invalid number of months in " + label + ". Please re-enter!");
    field.value = 0;
  }
}
...
{
	reloadForm(document.end, "miText", ":text:");
	reloadForm(document.end, "miSelect", ":select-one:");
	// alert(getCookie("LNoteScreen"));
	document.getElementById("lNoteScreen").value = decodeURI(getCookie("LNoteScreen"));
}


function saveCooks()
{
	saveForm(document.end, "miText", ":text:");
	saveForm(document.end, "miSelect", ":select-one:");		
	saveCookie("LNoteScreen", document.getElementById("lNoteScreen").value);
	//setCookieItem("miNotes", "LNoteScreen", document.getElementById("lNoteScreen").value);
}
...
/*function comaDel(a,lth,rex,bar,bat)
{
if(a.value==""){}
else{a.value = comaDel2(a.value,lth,rex,bar,bat);}
}
*/
...
function networthx()
{
	var assets = new Array("acib", "auto", "home", "other","rrsp","properties","deposit");
	var liabilities = new Array("finance", "mortgage", "card", "loan", "alimony", "mortgage2","auto");
 ...
 /*__________________DATA PRESENCE VALIDATION__________________*/
    ...
    function ControlSub(){
    	if(lendinfo==" "){
    		...
    		//LSAM - Sami commented hidFields call below
        //hidFields(352);
    		
    		//B2B Bank - Sami
    		var ack = document.getElementById("ack").checked;
    		if (ack!=true) {
    		  alert("Please acknowledge then submit!");
          return;
        }
			...
			{   
				...
				if(navigator.appVersion.substring(0,3) == 2.0 &&  navigator.appName.substring(0,8)=="Netscape" && (navigator.appVersion.indexOf("Macintosh")>=0|| navigator.appVersion.indexOf("PowerPC")>=0))
				{
					...
					{    						
						// alert("No Check 1!!");
						saveCooks();
						location.href = "../mortgage_default.asp";
					}
				} 
				else 
				{
...
function setBusHours()
{
  ...
  if(navigator.appVersion.substring(0,3) == 2.0 &&  navigator.appName.substring(0,8)=="Netscape" && (navigator.appVersion.indexOf("Macintosh")>=0|| navigator.appVersion.indexOf("PowerPC")>=0))
  {
    var todayMill = todayMill - 24*3600000;
    var minday = 24*3600000;
  }
                               

  //ROUND MINUTES TO THE NEAREST HALF HOUR
  if(todayMin >=0 && todayMin <=14){todayMin = 0;}
  if(todayMin >=15 && todayMin <=44){todayMin = 30;}
  if(todayMin >=45 && todayMin <=59){todayMin = 0; todayHrs = todayHrs+1; if(todayHrs==24){todayHrs=0;var plusDay =1;}}

  //FRIDAY AFTER 5 OR WEEKEND
  if(todayDay==0 || (todayDay==5 && todayHrs >=17) || todayDay==6)
  {
    if(todayDay==0){numDay = 1}
    if(todayDay==5){numDay = 3}
    if(todayDay==6){numDay = 2}
    var resendDay = new Date(todayMill + numDay*24*3600000);
    fivepm = 1;
  } 

  //WEEKDAY OR FRIDAY BEFORE 5
  else{
    //AFTER 9AM AND BEFORE 5PM 
    if(todayHrs<17 && todayHrs>=9)
    {
      if(todayDay == 5)
      {
        var resendDay = new Date(todayMill + 3*24*3600000);
      }
      else
      {
        var resendDay = new Date(todayMill + 1*24*3600000);
      }
      resendDay.setHours(todayHrs);
      resendDay = new Date(resendDay.getTime()-minday);
      resendDay.setMinutes(todayMin);
      resendDay = new Date(resendDay.getTime()-minday);
    }
    ...
    {
      if(todayHrs<9 && plusDay==0)
      {
        var resendDay = new Date(todayMill);
      }
      else
      {
        var resendDay = new Date(todayMill+86400000);
      }
      fivepm = 1;
    }
  }

  //SET HOURS AND MINUTES TO FIVE PM
  if(fivepm == 1)
  {
    resendDay.setHours(17);
    resendDay = new Date(resendDay.getTime()-minday);
    resendDay.setMinutes(0);
    resendDay = new Date(resendDay.getTime()-minday);
  }

  //SET SECONDS TO ZERO FOR ALL DATES
  resendDay.setSeconds(0);
  resendDay = new Date(resendDay.getTime()-minday);
  
  //STORES THE RESEND DATE IN A COOKIE WHICH EXPIRES IN 4 DAYS
  dateData = resendDay.getTime()-minday;
  var expire = new Date();
  expire.setTime (expire.getTime() + (numDateDays * 24 * 3600000));
  // document.cookie = chkDate +"=" + escape(dateData) + "; expires=" + expire.toGMTString()+";path=/";
  setCookieItem(chkDate, "busHours", escape(dateData));
}
...
{   
	...
	var todaytimeMill = todaytime.getTime();
	if(navigator.appVersion.substring(0,3) == 2.0 &&  navigator.appName.substring(0,8)=="Netscape" && (navigator.appVersion.indexOf("Macintosh")>=0|| navigator.appVersion.indexOf("PowerPC")>=0))
	{
		var todaytimeMill = todaytimeMill - 24*3600000;
	}
	
	var cookDate = getCookieItem("busHours", chkDate);
	if(cookDate!=null)
	{
		if(todaytimeMill >= unescape(cookDate))
		{
			setBusHours();
			return true;
		}
	//Set line below to else{return false;}
		else
		{
			return true;
		}
...
function  copyAddr()    
{
	var fields = new Array("Address1", "Address2", "Address3", "Address4", "CurrentUnitNum", "CurrentCity", "CurrentProvince", "CurrentCode", "HomePhone", "CurrentAddrTime");
	for(var i=0; i<fields.length; i++)
	{
...
if(browTest()==true){
Img = new Image();
Img.src = "https://www.secureapp.com/b2bbank/graphics/back_on.gif"; 
document.images.Back_button.src=Img.src;
}
else{}
...
if(browTest()==true){
OrImg= new Image();
OrImg.src="https://www.secureapp.com/b2bbank/graphics/back_off.gif";
document.images.Back_button.src=OrImg.src;
}
else{}
}
...
function go_french() {
  //location.href="https://www.secureapp.com/b2bbank_fr/app/cbasexmtgapp1.asp?expertusercode=&firmcode=&short=false";
	location.href="mtgapp_fr.html";
}
